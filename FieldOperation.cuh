...
__global__ void update_bv(cfd::DZone *zone, DParameter *param) {
  const integer extent[3]{zone->mx, zone->my, zone->mz}, ngg{zone->ngg};
  const auto i = (integer) (blockDim.x * blockIdx.x + threadIdx.x) - ngg;
  const auto j = (integer) (blockDim.y * blockIdx.y + threadIdx.y) - ngg;
  const auto k = (integer) (blockDim.z * blockIdx.z + threadIdx.z) - ngg;
  if (i >= extent[0] + ngg || j >= extent[1] + ngg || k >= extent[2] + ngg) return;
...
template<MixtureModel mix_model, TurbMethod turb_method>
__global__ void update_bv_2D(cfd::DZone *zone, DParameter *param) {
  const integer extent[3]{zone->mx, zone->my, zone->mz}, ngg{zone->ngg};
  const auto i = (integer) (blockDim.x * blockIdx.x + threadIdx.x) - ngg;
  const auto j = (integer) (blockDim.y * blockIdx.y + threadIdx.y) - ngg;
  constexpr auto k = 0;
  if (i >= extent[0] + ngg || j >= extent[1] + ngg) return;
  ...
  bv(i, j, k, 2) = cv(i, j, k, 2) * density_inv;
  bv(i, j, k, 3) = 0;
  velocity = bv(i, j, k, 1) * bv(i, j, k, 1) + bv(i, j, k, 2) * bv(i, j, k, 2); //V^2
...
template<MixtureModel mix_model, TurbMethod turb_method>
__device__ void update_bv_1point(cfd::DZone *zone, DParameter *param, integer i, integer j, integer k ) {
  auto &cv = zone->cv;
