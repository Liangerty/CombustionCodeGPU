...
DBoundCond<mix_model, turb_method>::apply_boundary_conditions_1st_step(const Block &block,
  ...
  for (size_t l = 0; l < n_outflow; l++) {
    ...
    for (size_t i = 0; i < nb; i++) {
      ...
      apply_outflow_1st_step<turb_method> <<<BPG, TPB>>>(field.d_ptr, i_face);
    }
  }
  ...
  for (size_t l = 0; l < n_inflow; l++) {
    ...
    for (size_t i = 0; i < nb; i++) {
      ...
      dim3 TPB{tpb[0], tpb[1], tpb[2]}, BPG{bpg[0], bpg[1], bpg[2]};
      apply_inflow_1st_step<mix_model, turb_method> <<<BPG, TPB>>>(field.d_ptr, &inflow[l], param, i_face);
    }
  }
  ...
  for (size_t l = 0; l < n_symmetry; l++) {
    ...
    for (size_t i = 0; i < nb; i++) {
      ...
      dim3 TPB{tpb[0], tpb[1], tpb[2]}, BPG{bpg[0], bpg[1], bpg[2]};
      apply_symmetry_1st_step<mix_model, turb_method> <<<BPG, TPB>>>(field.d_ptr, i_face);
    }
  }
  ...
  for (size_t l = 0; l < n_wall; l++) {
    ...
    for (size_t i = 0; i < nb; i++) {
      ...
      dim3 TPB{tpb[0], tpb[1], tpb[2]}, BPG{bpg[0], bpg[1], bpg[2]};
      apply_wall_1st_step<mix_model, turb_method><<<BPG, TPB>>>(field.d_ptr, &wall[l], param, i_face);
    }
  }
}
...
template<MixtureModel mix_model, TurbMethod turb_method>
__global__ void apply_symmetry_1st_step(DZone *zone, integer i_face) {
...
__global__ void apply_outflow_1st_step(DZone *zone, integer i_face) {
...
template<MixtureModel mix_model, TurbMethod turb_method>
__global__ void apply_wall_1st_step(DZone *zone, Wall *wall, DParameter *param, integer i_face) {
  const integer ngg = zone->ngg;
  integer dir[]{0, 0, 0};
  const auto &b = zone->boundary[i_face];
  dir[b.face] = b.direction;
